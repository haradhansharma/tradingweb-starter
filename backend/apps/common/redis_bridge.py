# # backend/apps/common/redis_bridge.py
"""
Redis Bridge
============
Bridges Binance WebSocket data → Redis cache → Django Channel Layer → Frontend.
...
class RedisBridge:
    ...
    async def broadcast(
        ...
        try:
            ...
            cache_key = f"binance:{underlying}:{category}"
            ...
            tasks.append(
                self.pubsub.publish(f"signal:{underlying}:{category}", raw_payload)
            )
    ...
    async def _generate_intelligence(
        ...
        try:
            asset = asset.upper()
            keys = [
                f"binance:{asset}:{CATEGORY_INDEX}",
                f"binance:{asset}:{CATEGORY_OPEN_INTEREST}",
                f"binance:{asset}:{CATEGORY_TRADE}",
            ]
            ...
            contract_sizes = {}
            try:
                ext_raw = await self.cache.get("binance:exchange_info")
                if ext_raw:
            ...
            intel_group = f"{asset}_{CATEGORY_INTELLIGENCE}"
            ...
            return [
                self.cache.set(
                    f"binance:{asset}:{CATEGORY_INTELLIGENCE}", intel_raw, ex=60
                ),
                self.channel_layer.group_send(
                    intel_group,
    ...
    async def _broadcast_error(self, underlying: str, message: str, category: str):
        ...
        try:
            ...
            for cat in [CATEGORY_MARK_PRICE, CATEGORY_INTELLIGENCE, CATEGORY_TRADE]:
                group = f"{underlying}_{cat}".replace("-", "_")
                try:
...
#             tasks = [
#                 # Cache normalized data with TTL
#                 self.cache.set(cache_key, raw_payload, ex=self._cache_ttl(category)),
#                 # Pub/Sub (raw for external consumers)
#                 self.pubsub.publish(f"signal:{underlying}:{category}", raw_payload),
#                 # Channel Layer group send
...
#             # Log any gather exceptions (don't let partial failures go silent)
#             # Note: return_exceptions=True means no exception is raised here,
#             # individual failures are in the results. We log them above.
