...
const VAR_FORMATTERS: Readonly<Record<string, VarFormatter>> = {
  ...
  max_wall: (v) => '$' + v.toLocaleString('en-US', { maximumFractionDigits: 0 }),
...
/** @internal Navigation item with optional badge and href */
interface NavItem {
  readonly id: string;
...
export function createDashboardStore(): DashboardStore {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      label: 'Credentials',
      href: '/credentials',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" class="w-[18px] h-[18px]"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>',
    },
  ];
  ...
  const sysItems: NavItem[] = [
    ...
    },
    {
      ...
      label: 'Profile',
      href: '/profile',
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" class="w-[18px] h-[18px]"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
    },
    {
      id: 'settings',
      label: 'Settings',
  ...
  const store: InternalDashboardStore = {
    ...
    darkMode: true,
    activeNav: (function() {
      // Set from URL path immediately — no waiting for init()
      const p = typeof window !== 'undefined' ? window.location.pathname : '/';
      return PATH_NAV_MAP[p] || 'dashboard';
    })(),
    ...
    async init() {
      // activeNav already set from URL path at creation time
    ...
    _setupWebSocket() {
      ...
      this.activeUnderlyings.forEach((symbol: string) => {
        ...
        ws.subscribe(symbol, 'futuresPrice', this.activeBroker);
      });
      ...
      // ── Futures live price handler (from futures kline tick stream) ──
      wsUnsubscribers.push(
        ws.on('data:futuresPrice', (payload: any) => {
          if (payload.broker && payload.broker !== this.activeBroker) return;
          const underlying = payload.underlying;
          const data = payload.data;
          if (!data) return;
    ...
    _updateAssetFromIntelligence(underlying: string, intel: any) {
      ...
      if (!this.assets[underlying]) {
        ...
        if (this.ws && this.connectionStatus === 'connected') {
          ...
          this.ws.subscribe(underlying, 'futuresPrice', this.activeBroker);
        }
      }
      ...
      asset.vars.max_wall = m.max_wall ?? null;
      // Debug: trace max_wall value to diagnose absence
      console.log(
        `[Dashboard] ${underlying} intelligence vars: max_pain=${m.max_pain}, max_wall=${m.max_wall}, ` +
        `support=${m.support}, resistance=${m.resistance}`
      );
      if ((asset.vars.max_wall === 0 || asset.vars.max_wall === null) && asset.vars.max_pain && asset.vars.max_pain > 0) {
        console.warn(
          `[Dashboard] ${underlying}: max_wall=${asset.vars.max_wall} but max_pain=${asset.vars.max_pain}. ` +
          `Backend OI data may be insufficient for max_wall calculation.`
        );
      }
    ...
    _removeAsset(underlying: string, reason?: string) {
      ...
      if (ws) {
        ...
        ws.unsubscribe(underlying, 'futuresPrice', this.activeBroker);
      }
    ...
    _emptyAsset(symbol: string): AssetState {
      const state: InternalAssetState = {
        ...
        vars: {
          ...
          max_pain: null, max_wall: null, support: null, support_secondary: null,
    ...
    switchBroker(broker: string) {
      ...
      if (ws && ws.readyState === WebSocket.OPEN) {
        this.activeUnderlyings.forEach((symbol: string) => {
          ...
          ws.unsubscribe(symbol, 'futuresPrice', oldBroker);
        });
      }
    ...
    showTooltip(group: string, key: string, fallbackGroup?: string, fallbackKey?: string, el?: HTMLElement): void {
      ...
      if (el) {
        ...
        // Estimated max tooltip height: title + 3 sections + padding ≈ 280px
        const tipEstHeight = 280;
        // Smart flip: flip below if not enough room above
        // When both sides are limited, pick whichever has more space
        if (spaceAbove < tipEstHeight && spaceBelow >= tipEstHeight) {
          // Not enough above, plenty below → flip
          this.tip.dir = 'below';
        } else if (spaceAbove < tipEstHeight && spaceBelow < tipEstHeight) {
          // Both sides limited → use the side with more room
          this.tip.dir = spaceBelow > spaceAbove ? 'below' : 'above';
        } else if (spaceBelow < tipEstHeight && spaceAbove > spaceBelow) {
          // Not enough below, plenty above → stay above
          this.tip.dir = 'above';
        } else {
          // Default: above
          this.tip.dir = 'above';
        }
        // Center horizontally on the trigger, clamped to keep tooltip within viewport
        const tipHalfWidth = 150; // half of w-[300px]
        this.tip.posX = Math.max(tipHalfWidth, Math.min(window.innerWidth - tipHalfWidth, rect.left + rect.width / 2));
