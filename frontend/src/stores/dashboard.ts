...
class MarketWebSocket {
   ...
   */
  subscribe(underlying: string, category: string) {
    const key = `${underlying}:${category}`;
    if (this.pendingSubscriptions.has(key)) return;

    const payload: WSSubscribePayload = { action: 'subscribe', underlying, category };
   ...
   */
  unsubscribe(underlying: string, category: string) {
    const key = `${underlying}:${category}`;
    this.pendingSubscriptions.delete(key);

    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      this._rawSend({ action: 'unsubscribe', underlying, category });
    }
  }
...
export function createDashboardStore(): DashboardStore {
  ...
  const store: InternalDashboardStore = {
    ...
    async _fetchActiveUnderlyings() {
      try {
        const resp = await fetch('/api/market/active-underlyings');
        if (!resp.ok) {
...
//OLD WORKING CODE FOR REFERENCE - NOT USED IN PRODUCTION
// /**
//  * WebSocket Service for MarketPulse Dashboard
//  * ==============================================
//  * Connects to Django Channels WebSocket at /ws/market/,
...
//     /** Saved subscription payloads for auto-resubscribe after reconnect */
...
//     this.heartbeatTimer = null;
//     this.reconnectTimer = null;
//     /** Last message timestamp for dead-connection detection */
...
//   /**
//    * Subscribe to a data stream.
//    * @param {string} underlying — e.g. "BTCUSDT"
//    * @param {'intelligence'|'index'|'markPrice'|'ticker'|'trade'|'open_interest'} category
//    */
//   subscribe(underlying, category) {
//     const key = `${underlying}:${category}`;
//     if (this.pendingSubscriptions.has(key)) return;

//     const payload = { action: 'subscribe', underlying, category };
//     this.pendingSubscriptions.set(key, payload);
...
//    */
//   unsubscribe(underlying, category) {
//     const key = `${underlying}:${category}`;
//     this.pendingSubscriptions.delete(key);
...
//   /** Start heartbeat monitor — checks for stale connection every 15s */
//   _startHeartbeat() {
//     this._stopHeartbeat();
//     this.heartbeatTimer = setInterval(() => {
//       if (this.ws && this.ws.readyState === WebSocket.OPEN) {
//         const elapsed = Date.now() - this.lastMessageTime;
//         if (elapsed > 90000) {
//           console.warn('[WS] No message received in 90s — connection is stale. Closing.');
//           this.ws.close(4000, 'Stale connection');
//         }
//       }
//     }, 15000);
...
//     async _fetchActiveUnderlyings() {
//       try {
//         const resp = await fetch('market/api/active-underlyings/');
//         const data = await resp.json();
//         this.activeUnderlyings = data.active_underlyings || ['BTCUSDT', 'ETHUSDT'];
//         this.activeUnderlyings.forEach((symbol: string) => {
//           if (!this.assets[symbol]) {
//             this.assets[symbol] = this._emptyAsset(symbol);
//           }
...
//       } catch (e) {
//         console.error('[Dashboard] Failed to fetch active underlyings:', e);
//         this.activeUnderlyings = ['BTCUSDT', 'ETHUSDT'];
//         this.activeUnderlyings.forEach((s: string) => {
//           if (!this.assets[s]) this.assets[s] = this._emptyAsset(s);
//         });
//       }
...
//       this.ws.on('connection_change', ({ status }: { status: string }) => {
//         this.connectionStatus = status as DashboardStore['connectionStatus'];
//       });

//       this.activeUnderlyings.forEach((symbol: string) => {
//         this.ws.subscribe(symbol, 'intelligence');
//         this.ws.subscribe(symbol, 'index');
//       });

//       // ── Intelligence data handler ──
//       this.ws.on('data:intelligence', (payload: any) => {
//         const underlying = payload.underlying;
//         const intel = payload.data;

//         if (intel.status === 'insufficient_data') return;

//         this._updateAssetFromIntelligence(underlying, intel);
//       });

//       // ── Index price handler ──
//       this.ws.on('data:index', (payload: any) => {
//         const data = payload.data;
//         if (!data) return;
...
//         if (this.assets[symbol]) {
//           const oldPrice = this.assets[symbol].price || 0;
//           this.assets[symbol].price = price;

//           if (oldPrice > 0 && this.assets[symbol].basePrice > 0) {
//             const pctChange = ((price - this.assets[symbol].basePrice) / this.assets[symbol].basePrice) * 100;
//             this.assets[symbol].change = parseFloat(pctChange.toFixed(2));

//             if (price > oldPrice) {
//               this.assets[symbol].flash = 'up';
//             } else if (price < oldPrice) {
//               this.assets[symbol].flash = 'down';
//             }
//             clearTimeout(this.assets[symbol]._flashTimer!);
//             this.assets[symbol]._flashTimer = setTimeout(() => {
//               if (this.assets[symbol]) this.assets[symbol].flash = null;
//             }, 600);
//           }
//         }
//       });
...
//       const buyVol = m.whale_buy_volume ?? 0;
//       const sellVol = m.whale_sell_volume ?? 0;
//       asset.vars.whale_net = (buyVol !== null && sellVol !== null) ? buyVol - sellVol : null;
//       asset.vars.max_pain = m.max_pain ?? null;
//       asset.vars.support = m.support ?? null;
//       asset.vars.support_secondary = m.support_secondary ?? null;
//       asset.vars.resistance = m.resistance ?? null;
...
//         oi: (v) => v.toLocaleString('en-US', { maximumFractionDigits: 0 }),
//       };
//       return formatters[key] ? formatters[key](v) : String(v);
//     },
...
//         'STRONG SELL': 'bg-red-400',
//         'CONNECTING': 'bg-sky-400',
//       } as Record<string, string>)[d] || 'bg-gray-400';
//     },
...
//         'STRONG SELL': 'bg-red-600 border-red-500 text-white',
//         'CONNECTING': 'bg-slate-500/60 border-slate-400/40 text-slate-200',
//       } as Record<string, string>)[d] || 'bg-gray-500/50 border-gray-400/30 text-gray-200';
//     },
...
//       if (this.ws) {
//         this.activeUnderlyings.forEach((s: string) => {
//           this.ws!.unsubscribe(s, 'intelligence');
//           this.ws!.unsubscribe(s, 'index');
//         });
//         this.ws.disconnect();
//       }
//     },
